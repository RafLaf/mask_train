...
def test(config, prunning_matrix=None):
    ...
    path = os.path.join(config.OUTPUT, "results_greedy.pt")
...
if __name__ == '__main__':
    ...
    if config.IS_TRAIN:
        ...
        start_acc = test(config, torch.ones((12, 12)))
        ...
        for k in range(144):
            ...
            for i in range(12):
                for j in range(12):
                    if not ((i,j) in L_best_pos): 
                        running_matrix[i][j] = 0
                        acc = test(config, running_matrix)
                        running_matrix[i][j] = 1
                        if acc > best_acc:
                            best_i, best_j = i, j
                            best_acc = acc
            if best_acc <= start_acc/1.7:
                break
            L_best_pos.append((best_i,best_j))
            for k,l in L_best_pos:
                running_matrix[k][l] = 0
