...
def test(config, prunning_matrix=None):
    ...
    path = os.path.join(config.OUTPUT, "results_double.pt")
...
if __name__ == '__main__':
    ...
    if config.IS_TRAIN:
        ...
        test(config, torch.ones((12, 12)))
        for i in range(0, 12):
            for j in range(0, 12):
                prunning_matrix = torch.ones((12, 12))
                prunning_matrix[11, j] = 0
                prunning_matrix[11, i] = 0
                test(config, prunning_matrix=prunning_matrix)
