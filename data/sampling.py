...
class EpisodeSampler(object):
  ...
  def sample_single_episode(self, sequential_sampling = False, replacement = False):
      ...
      if self.num_query is not None:
        ...
        print('max queries selected:', num_query)
  ...
  def sample_multiple_episode(self, batchsize, sequtial_sampling = False, replacement = False):
      all_images = []
      all_labels = []

      print('batch_size',batchsize)
      for task_index in range(batchsize):
        images, labels = self.sample_single_episode(sequential_sampling=sequtial_sampling, replacement=replacement)
        if images is None:
          return None, None
