...
def save_results(config, acc1, ci, prunning_matrix,heuristic=False,val_test=False, task_number=None):
    path = os.path.join(config.OUTPUT, f"test/results_val_set_{config.DATA.TEST.DATASET_NAMES[0]}_{config.DATA.TEST.EPISODE_DESCR_CONFIG.NUM_SUPPORT}_shots_task{task_number}.pt")
    ...
    if f"heuristic_{config.DATA.TEST.DATASET_NAMES[0]}" not in result_dic.keys():
        result_dic[f"heuristic_{config.DATA.TEST.DATASET_NAMES[0]}"] = []
    if f"val_test_{config.DATA.TEST.DATASET_NAMES[0]}" not in result_dic.keys():
        result_dic[f"val_test_{config.DATA.TEST.DATASET_NAMES[0]}"] = []
    if heuristic:
        result_dic[f"heuristic_{config.DATA.TEST.DATASET_NAMES[0]}"].append([acc1, ci, prunning_matrix])
    elif val_test:
        result_dic[f"val_test_{config.DATA.TEST.DATASET_NAMES[0]}"].append([acc1, ci, prunning_matrix])
    else:
        result_dic[f"{config.DATA.TEST.DATASET_NAMES[0]}"].append([acc1, ci, prunning_matrix]) #here acc1 is -loss
    torch.save(result_dic, path)
    logger.info(f"Results saved to {path}")


def test(config, prunning_matrix=None, task_number=None , heuristic=False, pre_loaded_data=None, val_set = False, test_set = False):
    ...
    with torch.no_grad():
        acc1, loss, ci = testing(config, test_dataset, test_dataloader, model, task_number, heuristic=heuristic , pre_loaded_data=pre_loaded_data, val_set=val_set, test_set=test_set)
        if heuristic:
    ...
    if not heuristic and not val_set:
...
@torch.no_grad()
def testing(config, dataset,data_loader, model, task_number=None, heuristic=False, pre_loaded_data=None, val_set = False, test_set = False):
    model.eval()
    ...
    if pre_loaded_data is not None:
        ...
        for idx, batches in enumerate(data_loader):
            if idx == task_number:
                ...
                break
    if heuristic==True:
        if len(imgs[0]['query'].shape) == 4:
            imgs[0]['query'] = imgs[0]['query'].unsqueeze(0)
            imgs[0]['support'] = imgs[0]['support'].unsqueeze(0)
            labels[0]['support'] = labels[0]['support'].unsqueeze(0)
            labels[0]['query'] = labels[0]['query'].unsqueeze(0)

        for i in range(labels[0]['support'].shape[1]):
            imgs_loo,labels_loo = loo(copy.deepcopy(imgs),copy.deepcopy(labels),i)
            if True:  #soft NCM (prototypical loss) not Cross entropy
                loss, acc =  soft_ncm(imgs_loo,labels_loo,model)
            else:  #cross entropy
                loss, acc = model.test_forward(imgs_loo, labels_loo, dataset_index)
                loss = -loss
            accs.extend(acc)
            acc = torch.mean(torch.stack(acc))
        ...
        if val_set:
            num_classes = torch.unique(labels[0]['support']).shape[0]
            # Get half of the items for each class
            num_samples_per_class = imgs[0]['query'].shape[0] // num_classes
            imgs[0]['query'] = torch.cat([imgs[0]['query'][i*num_samples_per_class:i*num_samples_per_class + num_samples_per_class//2] for i in range(num_classes)], dim=0)
            labels[0]['query'] = torch.cat([labels[0]['query'][i*num_samples_per_class:i*num_samples_per_class + num_samples_per_class//2] for i in range(num_classes)], dim=0)
            loss, acc = model.test_forward(imgs, labels, dataset_index)
        elif test_set:
            num_classes = torch.unique(labels[0]['support']).shape[0]
            # Get half of the items for each class
            num_samples_per_class = imgs[0]['query'].shape[0] // num_classes
            imgs[0]['query'] = torch.cat([imgs[0]['query'][i*num_samples_per_class+ num_samples_per_class//2:(i+1)*num_samples_per_class ] for i in range(num_classes)], dim=0)
            labels[0]['query'] = torch.cat([labels[0]['query'][i*num_samples_per_class+ num_samples_per_class//2:(i+1)*num_samples_per_class] for i in range(num_classes)], dim=0)
...
if __name__ == '__main__':
    ...
    if config.IS_TRAIN:
        ...
        start_acc = test(config, torch.ones((12, 12)),task_number=TN, pre_loaded_data=data_copy, test_set=True)
        logger.info(f"Initial accuracy: {start_acc}")
        running_matrix = torch.ones((12, 12))
        best_acc = 0
        best_i, best_j = 0, 0
        L_best_pos = []
        for k in range(15):
            L_loss, L_ci, L_prun = [], [], []
            best_acc = 0
            mapp_acc = np.zeros((12,12))
            for i in range(12):
                for j in range(12):
                    if running_matrix[i][j] == 1: 
                        running_matrix[i][j] = 0
                        data_copy = copy.deepcopy(data)
                        loss = test(config, running_matrix,task_number=TN, heuristic=False, pre_loaded_data=data_copy, val_set=True)
                        #acc = np.random.rand()
                        print(f'########## ; iteration (k) :{k}; i:{i},j:{j} ; loss: {loss}')
                        L_prun.append(running_matrix)
                        L_loss.append(loss)
                        L_ci.append(0)
                        running_matrix[i][j] = 1
                        mapp_acc[i][j] = loss
                    else:
                        mapp_acc[i][j] = -np.inf
            save_results(config, L_loss, L_ci, L_prun, heuristic=False, val_test=True, task_number=TN)
            print(mapp_acc)
            # Flatten the array
            flat_array = mapp_acc.flatten()

            # Get the indices that would sort the array
            sorted_indices = np.argsort(flat_array)

            # Select the last 10 indices (top 10 values)
            top_indices_flat = sorted_indices[-topK:]            
            top_indices = np.unravel_index(top_indices_flat, mapp_acc.shape)
            running_matrix[top_indices] = 0

            print(running_matrix)
            data_copy = copy.deepcopy(data)
            acc = test(config, running_matrix,task_number=TN, heuristic=False, pre_loaded_data=data_copy,test_set=True)
