...
def test(config, prunning_matrix=None):
    ...
    path = os.path.join(config.OUTPUT, "results_exhaustiv.pt")
    ...
    if f"{config.DATA.TEST.DATASET_NAMES[0]}" not in result_dic.keys():
        result_dic[f"{config.DATA.TEST.DATASET_NAMES[0]}"] = []
    result_dic[f"{config.DATA.TEST.DATASET_NAMES[0]}"].append([acc1, ci, prunning_matrix])
...
def recursive_prune(prn_mat,k):
    if k == 0:
        print(prn_mat)
        return test(config, prn_mat)
    for i in range(0, 12):
        for j in range(0, 12):
            if prn_mat[i, j] == 1:
                prn_mat[i, j] = 0
                recursive_prune(prn_mat, k-1)
                prn_mat[i, j] = 1
...
if __name__ == '__main__':
    ...
    if config.IS_TRAIN:
        ...
        small=False
        for dat in ['dtd', 'vgg_flower', 'aircraft', 'cub']:
            print(dat, config.DATA.TEST.DATASET_NAMES, dat in config.DATA.TEST.DATASET_NAMES)
            if dat in config.DATA.TEST.DATASET_NAMES:
                small=True
        n_iter = 4 if small else 3
        print(n_iter)
        for k in range(4):
            recursive_prune(torch.ones((12, 12)), k)
