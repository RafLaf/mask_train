...
def save_results(config, acc1, ci, prunning_matrix,heuristic=False, task_number=None):
    path = os.path.join(config.OUTPUT, f"results_heuristics_{config.DATA.TEST.DATASET_NAMES[0]}_{config.DATA.TEST.EPISODE_DESCR_CONFIG.NUM_SUPPORT}_shots_task{task_number}.pt")
...
def testing(config, dataset,data_loader, model, task_number=None, heuristic=False, pre_loaded_data=None):
    ...
    if heuristic==True:
        ...
        if True:
            imgs_heur , labels_heur = copy.deepcopy(imgs),copy.deepcopy(labels)
            imgs_heur[0]['query'] = imgs_heur[0]['support']
            labels_heur[0]['query'] = labels_heur[0]['support']
            loss, acc = model.test_forward(imgs_heur, labels_heur, dataset_index)
...
if __name__ == '__main__':
    ...
    if config.IS_TRAIN:
        ...
        start_acc = test(config, torch.ones((12, 12)),task_number=TN, pre_loaded_data=data_copy)
        ...
        for k in range(15):
            ...
            for i in range(12):
                for j in range(12):
                    ...
                    if running_matrix[i][j] == 1: 
                        ...
                        data_copy = copy.deepcopy(data)
                        loss = test(config, running_matrix,task_number=TN, heuristic=True, pre_loaded_data=data_copy)
                        #acc = np.random.rand()
                    ...
                    count +=1 
                    avg  = avg * (count-1)/count + (end_loop-start_loop)/count
                    logger.info(f"avg Time taken for loop: {avg}")
            save_results(config, L_loss, L_ci, L_prun, heuristic=True, task_number=TN)
            print(mapp_acc)
            # Flatten the array
            ...
            data_copy = copy.deepcopy(data)
            acc = test(config, running_matrix,task_number=TN, heuristic=False, pre_loaded_data=data_copy)
